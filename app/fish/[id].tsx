import {
 ...
 const allData = {
  ...
  export default function FishDetail() {
    ...
    const cuts = ["Cut & Cleaned", "Fillet", "Whole"];
  
    return (
      <LinearGradient
        colors={["#ffe4e6", "#fcdada", "#fff1f2"]}
        start={{ x: 0, y: 0 }}
        end={{ x: 1, y: 1 }}
        locations={[0, 0.5, 1]}
        style={{ flex: 1 }}
          ...
          <ScrollView contentContainerStyle={{ paddingBottom: 120 }}>
            ...
            <View style={styles.optionRow}>
              {weights.map((w) => (
                <TouchableOpacity
                  key={w}
                  style={[styles.optionBtn, weight === w && styles.activeBtn]}
                  onPress={() => setWeight(w)}
                >
            ...
            <View style={styles.optionRow}>
              {cuts.map((cut) => (
                <TouchableOpacity
                  key={cut}
                  style={[styles.optionBtn, cutType === cut && styles.activeBtn]}
                  onPress={() => setCutType(cut)}
                >
